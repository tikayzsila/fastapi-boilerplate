...
async def login_for_token(user: LoginUser, request: Request) -> dict[str, str]:
...
async def get_users(request: Request) -> GetAllUsers:
    users_data = await DBUser.objects.values(["user_id", "login"])
    return GetAllUsers(users=users_data)


@users_router.put("/{id}", response_model=User, dependencies=[Depends(j.get_current_user)])
...
@users_router.get("/{id}", response_model=User, dependencies=[Depends(j.get_current_user)])
